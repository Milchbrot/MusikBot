...
class MusicBot(discord.Client):
    ...
    def owner_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not orig_msg or orig_msg.author.id == self.config.owner_id:
                ...
                raise exceptions.PermissionsError("Nur aPinat kann diesen Command benutzen.", expire_in=30)
    ...
    async def _auto_summon(self):
        ...
        if owner:
            self.safe_print("aPinat in \"%s\", joine..." % owner.voice_channel.name)
            # TODO: Effort
    ...
    async def _check_ignore_non_voice(self, msg):
        ...
        if not vc or vc == msg.author.voice_channel:
            ...
            raise exceptions.PermissionsError(
                "Du kannst diesen Command nicht benutzen, wenn du nicht im Sprach-Kanal bist (%s)" % vc.name, expire_in=30)
    ...
    async def get_voice_client(self, channel):
        ...
        if getattr(channel, 'type', ChannelType.text) != ChannelType.voice:
            raise AttributeError('Kanal muss ein Sprach-Kanal sein')
        ...
        if server.id not in self.players:
            if not create:
                raise exceptions.CommandError(
                    'Bot ist in keinem Sprach-Kanal.  '
                    'Benutze %ssummon, um ihn in deinen Sprach-Kanal zu rufen.' % self.config.command_prefix)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                "Du hast dein Song-Warteschlangenlimit erreicht (%s)" % permissions.max_songs, expire_in=30
            )
        ...
        if not info:
            raise exceptions.CommandError("Dieses Video kann nicht abgespielt werden.", expire_in=30)
        ...
        if 'entries' in info:
            ...
            if not permissions.allow_playlists and ':search' in info['extractor'] and len(info['entries']) > 1:
                raise exceptions.PermissionsError("Du darfst keine Playlisten hinzufügen", expire_in=30)
            ...
            if permissions.max_playlist_length and num_songs > permissions.max_playlist_length:
                raise exceptions.PermissionsError(
                    "Playlist hat zu viele Einträge (%s > %s)" % (num_songs, permissions.max_playlist_length),
                    expire_in=30
                )
            ...
            if permissions.max_songs and player.playlist.count_for_user(author) + num_songs > permissions.max_songs:
                raise exceptions.PermissionsError(
                    "Playlist-Einträge + deine Songs in der Warteschlange haben das Limit erreicht (%s + %s > %s)" % (
            ...
            if info['extractor'].lower() in ['youtube:playlist', 'soundcloud:set', 'bandcamp:album']:
                try:
                    ...
                    traceback.print_exc()
                    raise exceptions.CommandError("Fehler beim einsetzten der Playlist:\n%s" % e, expire_in=30)
            ...
            procmesg = await self.safe_send_message(
                channel,
                'Suche Playlist-Information für {} Songs{}'.format(
                    num_songs,
                    ', ETA: {} Sekunden'.format(self._fixg(
            ...
            if permissions.max_song_length:
                ...
                if drop_count:
                    print("Entferne %s Songs" % drop_count)

            print("Durchlaufe {} Songs in {} Seconds mit {:.2f}s/Song, {:+.2g}/Song von Erwarteten ({}s)".format(
                listlen,
                self._fixg(ttime),
                ttime / listlen,
            ...
            if not listlen - drop_count:
                raise exceptions.CommandError(
                    "Keine Songs hinzugefügt, alle zu lang (%ss)" % permissions.max_song_length,
                    expire_in=30
                )

            reply_text = "**%s** Songs in der Warteschlange eingefügt. Position in der Warteschlange: %s"
            btext = str(listlen - drop_count)

        else:
            if permissions.max_song_length and info.get('duration', 0) > permissions.max_song_length:
                raise exceptions.PermissionsError(
                    "Song zu lang (%s > %s)" % (info['duration'], permissions.max_song_length),
                    expire_in=30
                )
            ...
            reply_text = "**%s** in die Warteschlange eingefügt. Position in der Warteschlange: %s"
            btext = entry.title

        if position == 1 and player.is_stopped:
            position = 'Nächstes Lied!'
            reply_text %= (btext, position)

        else:
            try:
                time_until = await player.playlist.estimate_time_until(position, player)
                reply_text += ' - ETA bis zum Abspielen: %s'
            except:
                traceback.print_exc()
                time_until = ''
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not info:
            raise exceptions.CommandError("Diese Playlist kann nicht abgespielt werden.")

        num_songs = sum(1 for _ in info['entries'])
        t0 = time.time()

        busymsg = await self.safe_send_message(
            channel, "Durchsuche %s songs..." % num_songs)  # TODO: From playlist_title
        await self.send_typing(channel)
        ...
        if extractor_type == 'youtube:playlist':
            try:
                ...
                traceback.print_exc()
                raise exceptions.CommandError('Fehler in der Verarbeitung von %s.' % playlist_url, expire_in=30)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) > permissions.max_songs:
            raise exceptions.PermissionsError(
                "Du hast dein Playlist-Item Limit erreicht (%s)" % permissions.max_songs,
                expire_in=30
            )
        ...
        try:
            ...
            raise exceptions.CommandError("Zitiere deinen Suchbegriff richtig.", expire_in=30)
        ...
        if leftover_args[0].isdigit():
            ...
            if items_requested > max_items:
                raise exceptions.CommandError("Du kannst nicht für mehr als %s Videos suchen" % max_items)
        ...
        search_msg = await self.send_message(channel, "Suche nach Videos...")
        await self.send_typing(channel)
        ...
        if not info:
            return Response("Keine Videos gefunden.", delete_after=30)
        ...
        for e in info['entries']:
            result_message = await self.safe_send_message(channel, "Ergebnis %s/%s: %s" % (
                info['entries'].index(e) + 1, len(info['entries']), e['webpage_url']))

            confirm_message = await self.safe_send_message(channel, "Ist das ok? Tippe `y`, `n` or `exit`")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return Response("Ok egal.", delete_after=30)
            ...
            if response_message.content.lower().startswith('y'):
                ...
                return Response("Ok, nächstes Lied!", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)

        return Response("Naja :frowning:", delete_after=30)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Aktueller Titel: **%s** hinzugefügt von **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str)
            else:
                np_text = "Aktueller Titel: **%s** %s\n" % (player.current_entry.title, prog_str)

            self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, np_text)
            await self._manual_delete_check(message)
        else:
            return Response(
                'Keine Lieder laufen! Spiel etwas mit {}play ab.'.format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('Du bist in keinem Sprach-Kanal!')
        ...
        if not chperms.connect:
            self.safe_print("Kann nicht in \"%s\" joinen, keine Rechte." % author.voice_channel.name)
            return Response(
                "```Kann nicht in \"%s\" joinen, keine Rechte.```" % author.voice_channel.name,
                delete_after=25
            )

        elif not chperms.speak:
            self.safe_print("Werde nicht in \"%s\" joinen, keine Rechte zum Sprechen." % author.voice_channel.name)
            return Response(
                "```Werde nicht in \"%s\" joinen, keine Rechte zum Sprechen.```" % author.voice_channel.name,
                delete_after=25
            )
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError('Bot spielt nicht.', expire_in=30)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError('Bot ist nicht pausiert.', expire_in=30)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if player.is_stopped:
            raise exceptions.CommandError("Kann nicht skippen! Bot spielt nicht!", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    # print(player.playlist.peek()._waiting_futures[0].__dict__)
                    return Response("Nächster Song (%s) lädt, bitte warten." % player.playlist.peek().title)

                elif player.playlist.peek().is_downloaded:
                    print("Nächster Song läuft gleich.  Bitte warten.")
                else:
        ...
        if skips_remaining <= 0:
            player.skip()  # check autopause stuff here
            return Response(
                'Dein Skip für **{}** ist angekommen.'
                '\Die Abstimmung zum Skip ist durch.{}'.format(
                    player.current_entry.title,
                    ' Nächster Song kommt!' if player.playlist.peek() else ''
                ),
                reply=True,
                delete_after=20
            )
            ...
            return Response(
                'Dein Skip für **{}** ist angekommen.'
                '\nNoch **{}** {} gebraucht zum Skippen.'.format(
                    player.current_entry.title,
                    skips_remaining,
                    'Person' if skips_remaining == 1 else 'Personen'
                ),
                reply=True,
                delete_after=20
            )
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response('Aktuelle Lautstärke: `%s%%`' % int(player.volume * 100), reply=True, delete_after=20)
        ...
        try:
            ...
            raise exceptions.CommandError('{} ist keine gültige Zahl'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('Lautstärke von %d zu %d' % (old_volume, new_volume), reply=True, delete_after=20)

        else:
            if relative:
                raise exceptions.CommandError(
                    'Ungültige Lautstärkenänderung eingegeben: {}{:+} -> {}%.  Gib eine Änderung zwischen {} und {:+} an.'.format(
                        old_volume, vol_change, old_volume + vol_change, 1 - old_volume, 100 - old_volume), expire_in=20)
            else:
                raise exceptions.CommandError(
                    'Ungültige Lautstärke: {}%. Gib einen Wert zwischen 1 und 100 ein.'.format(new_volume), expire_in=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        lines = []
        unlisted = 0
        andmoretext = '* ... und %s mehr*' % ('x' * len(player.playlist.entries))
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Aktueller Titel: **%s** hinzugefügt von **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append("Aktueller Titel: **%s** %s\n" % (player.current_entry.title, prog_str))

        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = '`{}.` **{}** hinzugefügt von **{}**'.format(i, item.title, item.meta['author'].name).strip()
            else:
        ...
        if not lines:
            lines.append(
                'Keine Songs in der Warteschlange! Spiele etwas mit {}play ab.'.format(self.config.command_prefix))
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response('{} Nachricht{} gelöscht.'.format(len(deleted), 'en' * bool(deleted)), delete_after=15)
        ...
        return Response('{} Nachricht{} gelöscht.'.format(deleted, 'en' * bool(deleted)), delete_after=15)
    ...
    async def cmd_pldump(self, channel, song_url):
        ...
        try:
            ...
            raise exceptions.CommandError("Kann keine Info von URL holen \n%s\n" % e, expire_in=25)

        if not info:
            raise exceptions.CommandError("Kann keine Info von URL holen, keine Daten.", expire_in=25)
        ...
        if not info.get('entries', None):
            ...
            if info.get('url', None) != info.get('webpage_url', info.get('url', None)):
                raise exceptions.CommandError("Das scheint keine Playlist zu sein.", expire_in=25)
            else:
        ...
        if not exfunc:
            raise exceptions.CommandError("Kann keine Info von URL holen; wird nicht unterstützt.", expire_in=25)
        ...
        with BytesIO() as fcontent:
            ...
            fcontent.seek(0)
            await self.send_file(channel, fcontent, filename='playlist.txt', content="Hier ist der URL-Dump von <%s>" % song_url)
    ...
    async def cmd_setnick(self, server, channel, leftover_args, nick):
        ...
        if not channel.permissions_for(server.me).change_nickname:
            raise exceptions.CommandError("Kann nick nicht ändern: keine Rechte.")
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        try:
            ...
            raise exceptions.CommandError("Kann Avatar nicht ändern: %s" % e, expire_in=20)
    ...
    async def on_message(self, message):
        ...
        if message.author == self.user:
            self.safe_print("Ignoriere Nachricht von mir selbst (%s)" % message.content)
            return
        ...
        if message.channel.is_private:
            if not (message.author.id == self.config.owner_id and command == 'joinserver'):
                await self.send_message(message.channel, 'Du kannst den Bot nicht in PMs benutzen.')
                return

        if message.author.id in self.blacklist and message.author.id != self.config.owner_id:
            self.safe_print("[Benutzer geblacklistet] {0.id}/{0.name} ({1})".format(message.author, message_content))
            return
        ...
        try:
            ...
            if message.author.id != self.config.owner_id:
                if user_permissions.command_whitelist and command not in user_permissions.command_whitelist:
                    raise exceptions.PermissionsError(
                        "Dieser Command ist in deiner Gruppe nicht verfügbar (%s)." % user_permissions.name,
                        expire_in=20)
